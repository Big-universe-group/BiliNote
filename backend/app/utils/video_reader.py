...
class VideoReader:
    ...
    def group_images(self) -> list[list[str]]:
        ...
        image_files.sort(
            key=lambda f: self.extract_time_from_filename(os.path.basename(f))
        )
        group_size = self.grid_size[0] * self.grid_size[1]
        return [
            image_files[i : i + group_size]
    ...
    def concat_images(self, image_paths: list[str], name: str) -> str:
        ...
        font = (
            ImageFont.truetype(self.font_path, 48)
        ...
        for path in image_paths:
            img = (
                Image.open(path)
                .convert("RGB")
                .resize((self.unit_width, self.unit_height), Image.Resampling.LANCZOS)
            )
            timestamp = re.search(r"frame_(\d{2})_(\d{2})\.jpg", os.path.basename(path))
            time_text = (
                f"{timestamp.group(1)}:{timestamp.group(2)}" if timestamp else ""
            )
        ...
        grid_img = Image.new(
            "RGB", (self.unit_width * cols, self.unit_height * rows), (255, 255, 255)
        )
        ...
        try:
            ...
            for idx, group in enumerate(groups, start=1):
                if len(group) < self.grid_size[0] * self.grid_size[1]:
                    logger.warning(
                        f"⚠️ 跳过第 {idx} 组，图片不足 {self.grid_size[0] * self.grid_size[1]} 张"
                    )
                    continue
