...
if exist frontend\src-tauri\bin rmdir /s /q frontend\src-tauri\bin
echo 清理完成。

REM 重新创建 Tauri 需要的目录结构
mkdir frontend\src-tauri\bin
...
pyinstaller ^
  ...
  --paths backend ^
  --distpath frontend\src-tauri\bin ^
...
move /Y frontend\src-tauri\bin\BiliNoteBackend\BiliNoteBackend.exe frontend\src-tauri\bin\BiliNoteBackend\BiliNoteBackend-%TARGET_TRIPLE%.exe

echo PyInstaller 打包完成：
dir frontend\src-tauri\bin\BiliNoteBackend

echo 请检查 frontend\src-tauri\bin\BiliNoteBackend 目录，确认其中包含了名为 .env 的【文件】。
