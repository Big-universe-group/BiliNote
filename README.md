...
- 笔记顶部视频封面 Banner 展示
- 工作区和生成历史面板支持折叠/展开
- **异步非阻塞任务**：提交后立即可继续操作，多任务并行排队生成
- **生成历史多选**：支持多选笔记后批量删除、合并导出
- **历史排序**：按创建时间、标题、发布日期排序，支持正序/倒序切换
- **多篇笔记合并导出**：将多条历史笔记合并为一个 Markdown 文件下载
- **UP 主空间批量提取**：输入 UP 主主页链接，自动抓取空间内所有视频，支持按时间、播放量过滤，分页浏览
- **批量生成笔记**：从空间视频列表多选，一键批量提交笔记生成任务
...
- 性能优化与转写器配置改进
- 异步非阻塞任务生成，多任务排队不卡界面
- 生成历史支持多选、排序、批量删除、合并导出 Markdown
- UP 主空间页面：批量提取视频链接并支持批量生成笔记
